...
class TestDataHandler:
    ...
    def test_load_invalid_format(self):
        ...
        with pytest.raises(ValueError, match="Unsupported file format"):
            handler.load_dataset("invalid.txt")
    ...
    def test_preprocess_and_split(self, temp_pkl_file):
        ...
        # Check split sizes (default 60/20/20)
        total = len(X_train) + len(X_val) + len(X_test)
        assert total == 100, "Total samples should match original"
        assert len(X_train) == 60, "Train should be 60%"
        assert len(X_val) == 20, "Val should be 20%"
