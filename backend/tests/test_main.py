...
class TestUploadEndpoint:
    ...
    def test_upload_api_prefix(self, valid_pkl_file):
        """Test upload with /api prefix"""
        with open(valid_pkl_file, 'rb') as f:
            response = client.post(
                "/api/upload",
                files={"file": ("test.pkl", f, "application/octet-stream")}
...
class TestTrainEndpoint:
    ...
    def test_train_with_data(self, uploaded_dataset):
        ...
        assert "val_r2" in data["metrics"]
        assert data["metrics"]["val_r2"] > 0  # Should have some predictive power
